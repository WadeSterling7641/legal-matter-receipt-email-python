...
> Design notes for **legal-matter-receipt-email-python** — a runnable python example that Typed Python service for signed-document receipt emails and delivery lookup.
...
- **Feature code** — the actual task: legal matter receipts.
...
- `email.get` — mapped to `GET /v1/email/get/{id}`.
